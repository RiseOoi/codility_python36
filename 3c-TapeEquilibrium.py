...
# O(2N) time (summing and looping) and O(1) space
def solution(A):
    # a straightforward solution, get the left hand side and right hand side
    left = A[0]
    right = sum(A[1:])
    ret = abs(left - right)

    # then just loop over the array (excluding the first and last element)
    # hopefully no extra explanation needed
    for i in A[1:-1]:
        left += i
        right -= i
        ret = min(abs(left - right), ret)
